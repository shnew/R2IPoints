...
class ModulatedDeformConvFunction(Function):
    ...
    def forward(ctx,
        ...
        if isinstance(ctx.padding, int):
            ...
            deform_conv_cuda.modulated_deform_conv_cuda_forward(
                input, weight, bias, ctx._bufs[0], offset, mask, output,
                ctx._bufs[1], weight.shape[2], weight.shape[3], ctx.stride[0],
                ctx.stride[1], ctx.padding[0], ctx.padding[1], ctx.dilation, ctx.dilation,
                ctx.groups, ctx.deformable_groups, ctx.with_bias)
        #original
        # deform_conv_cuda.modulated_deform_conv_cuda_forward(
        #     input, weight, bias, ctx._bufs[0], offset, mask, output,
    ...
    def backward(ctx, grad_output):
        ...
        if isinstance(ctx.padding, int):
            ...
            deform_conv_cuda.modulated_deform_conv_cuda_backward(
                input, weight, bias, ctx._bufs[0], offset, mask, ctx._bufs[1],
                grad_input, grad_weight, grad_bias, grad_offset, grad_mask,
                grad_output, weight.shape[2], weight.shape[3], ctx.stride[0],
                ctx.stride[1], ctx.padding[0], ctx.padding[1], ctx.dilation, ctx.dilation,
                ctx.groups, ctx.deformable_groups, ctx.with_bias)
        # original
        # deform_conv_cuda.modulated_deform_conv_cuda_backward(
    ...
    def _infer_shape(ctx, input, weight):
        ...
        if isinstance(ctx.padding, int):
            ...
            height_out = (height + 2 * ctx.padding[0] -
                          (ctx.dilation * (kernel_h - 1) + 1)) // ctx.stride[0] + 1
            width_out = (width + 2 * ctx.padding[1] -
                         (ctx.dilation * (kernel_w - 1) + 1)) // ctx.stride[1] + 1
        #original
        # height_out = (height + 2 * ctx.padding -
        #               (ctx.dilation * (kernel_h - 1) + 1)) // ctx.stride + 1
        # width_out = (width + 2 * ctx.padding -
