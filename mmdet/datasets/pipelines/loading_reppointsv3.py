...
class LoadRPDV3Annotations(object):
    ...
    def __init__(self, num_classes=20):
        super(LoadRPDV3Annotations, self).__init__()
    ...
    def _load_semantic_map_from_box(self, results):
        ...
        # sh
        if (gt_bboxes_w == 0).sum():
            gt_bboxes_w[gt_bboxes_w == 0] = 1
        if (gt_bboxes_h == 0).sum():
            gt_bboxes_h[gt_bboxes_h == 0] = 1
        # end
        # gt_areas = (gt_bboxes[:, 2] - gt_bboxes[:, 0]) * (gt_bboxes[:, 3] - gt_bboxes[:, 1])
        gt_areas = gt_bboxes_h * gt_bboxes_w
        ...
        for ind in indexs[::-1]:
            box = gt_bboxes[ind]
            #sh
            gt_bboxes_xy = (box[:2] + box[2:]) / 2
            gt_bboxes_wh = (box[2:] - box[:2])
            scale_gt_bboxes_wh = gt_bboxes_wh * 0.5  # out   0.5
            new_gt_xy1 = gt_bboxes_xy - scale_gt_bboxes_wh * 0.5
            new_gt_xy2 = gt_bboxes_xy + scale_gt_bboxes_wh * 0.5
            box = np.concatenate((new_gt_xy1, new_gt_xy2),axis=0)
            #
            ...
            if gt_labels[ind] == 20 :
                ss = 1
            gt_sem_map[gt_labels[ind] - 1][box_mask > 0] = 1
            gt_sem_weights[gt_labels[ind] - 1][box_mask > 0] = 1 / gt_areas[ind]
...
@PIPELINES.register_module()
class LoadDenseRPDV3Annotations(object):
    """Load mutiple types of annotations.
    ...
    """
    def __init__(self):
        super(LoadDenseRPDV3Annotations, self).__init__()
